...
A = 2;
%% Plot model responses for varying relative weightings
% Time constants
tau1list = [40, 40, 40];
...
plist = [0, 0.1, 0.2];
% Plot model response
plotRisetimeBarlowLevickDiracCombResponse(vRange, spatialPeriod, dt, M, A, tau1list, tau2list, tau2ilist, plist);

%% Utility functions 
function plotRisetimeBarlowLevickDiracCombResponse(vRange, spatialPeriod, dt, M, A, tau1list, tau2list, tau2ilist, plist)

if (length(tau1list) ~= length(tau2list)) || (length(tau1list) ~= length(tau2ilist)) || (length(tau1list) ~= length(plist)) 
...
for indT = 1:length(tau1list) 
    ...
    f1 = @(t) max(0,t) .* exp(-max(0,t) ./ tau1list(indT)) ./ tau1list(indT).^2;
    f2 = @(t) (1-plist(indT))*max(0,t).*exp(-max(0,t)/tau2list(indT))/tau2list(indT)^2 + plist(indT)*max(0,t).*exp(-max(0,t)/tau2ilist(indT))/tau2ilist(indT)^2;
...
legend(strip(cellstr(num2str([tau1list',tau2list', tau2ilist', plist'],'\\tau_1 = %d ms, \\tau_2 = %d ms, \\tau_{2,i} = %d ms, p = %0.2f'))));
axis('square');
